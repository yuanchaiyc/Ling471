...
# TODO_1
vowel_a_mu = numpy.mean(vowel_a)
# TODO_1
vowel_a_sigma = numpy.std(vowel_a, ddof=1)
...
mn_sigma = numpy.std(vowel_all, ddof=1)
def numerator(x, category):
    ...
    # Calculate P(x | Y = K) K is the vowel a
        ...
        a_dens = scipy.stats.norm.pdf(x, vowel_a_mu, mn_sigma)
...
def denominator(x):
    ...
    return numerator(x, 'i') + numerator(x, 'a')
...
def classify(x):
    ...
    p = custom_lda(x)
    #if given the F1 value, the probability of being in iy category is higher, classify this datapoint as iy
    if p[0] > p[1]:
        return 'i'
    #else if given the F1 value, the probability of being in ih category is higher, classify the datapoint as ih
    else:
        return 'a'
