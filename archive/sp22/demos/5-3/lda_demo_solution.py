...
# TODO_1
vowel_iy_mu = numpy.mean(vowel_iy)
# TODO_1
vowel_iy_sigma = numpy.std(vowel_iy, ddof=1)

# TODO_1
vowel_ih_mu = numpy.mean(vowel_ih)
# TODO_1
vowel_ih_sigma = numpy.std(vowel_ih, ddof=1)
...
def numerator(x, category):
    ...
    if category == 'iy':
        iy_dens = scipy.stats.norm.pdf(x, vowel_iy_mu, mn_sigma)
        return iy_dens * py_given_k
    else:
        ih_dens = scipy.stats.norm.pdf(x, vowel_ih_mu, mn_sigma)
        return ih_dens * py_given_k
        
def denominator(x):

    return numerator(x, 'iy') + numerator(x, 'ih')

def custom_lda(x):

    d = denominator(x)

    p_iy = numerator(x, 'iy') / d
    p_ih = numerator(x, 'ih') / d
